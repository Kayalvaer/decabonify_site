...
def all_products(request):
    """Sort all solutions and search queries"""
...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            product = form.save()
            messages.success(request, 'Welldone! Product added successfully.')
            return redirect(reverse('product_detail', args=[product.id]))
        else:
            messages.error(request, 'Not able to add product. Check your form validity.')
    else:
        form = ProductForm()
...
def edit_product(request, product_id):
    ...
    if request.method == 'POST':
        ...
        messages.info(request, f'Editing solution {product.name}')
